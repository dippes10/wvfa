  ...
  return (
    <>
      <motion.div
        ...
        className={cn(
          "absolute left-0 top-20 m-auto h-14 w-px rounded-full bg-gradient-to-t from-foreground via-primary to-transparent",
...
const Explosion = ({ ...props }: React.HTMLProps<HTMLDivElement>) => {
  // Randomized once per mount (useState initializer), not recomputed on every
  // render - Math.random() during render is an impure-render violation.
    ...
    <div {...props} className={cn("absolute z-50 h-2 w-2", props.className)}>
      ...
      {spans.map((span) => (
        <motion.span
          ...
          className="absolute h-1 w-1 rounded-full bg-gradient-to-b from-primary to-foreground"
        />
      ))}
    </div>
  );
